...
//     else {
//         /* ── Detect wrong keyword using edit distance ── */
//         int dist = 0;
//         const char *nearest = closestKeyword(token.lexeme, &dist);

//         /* 🔥 SIMPLE & RELIABLE RULE */
//         if (nearest && dist <= 2) {
//             fprintf(stderr,
//                 "[Lexical Error] Line %d: '%s' is invalid — did you mean '%s'?\n",
//                 token.line, token.lexeme, nearest);
...
if (isalpha(c)) {
    ...
    return token;
            ...
            //      *
            //      * We flag a TOKEN_INVALID (lexical error) when ALL of:
            //      *   1. The word is at least 2 chars long.
            //      *   2. Its edit distance from some keyword is ≤ 2.
            //      *   3. The edit distance is < len(word)/2
            //      *      (so "a" or "ab" don't falsely match "if"/"int").
            //      *
            //      * Anything that doesn't satisfy these is a normal identifier.
            //      * ──────────────────────────────────────────────────────
            //      */
            //     int wordLen = (int)strlen(token.lexeme);
            //     int dist    = 0;
            //     const char *nearest = closestKeyword(token.lexeme, &dist);

            //     int threshold = 2;                      /* allow 1 or 2 edits  */
            //     /* For very short words (len<=3) be stricter: only 1 edit  */
            //     if (wordLen <= 3) threshold = 1;

            //     if (nearest && dist > 0 && dist <= threshold
            //         && dist < wordLen)   /* never match a word to itself   */
            //     {
            //         fprintf(stderr,
            //             "[Lexical Error] Line %d: '%s' is not a valid keyword "
            //             "or identifier — did you mean '%s'?\n",
            //             token.line, token.lexeme, nearest);
            //         lexErrorCount++;
            //         token.type = TOKEN_INVALID;
            //     } else {
